...
if [[ -z "${ENV_FILE}" || ! -f "${ENV_FILE}" ]]; then
  echo "[ERROR] 请先运行 prepare_dataset.sh，然后传入 env.sh 路径！"
  exit 1
fi
...
RAW_DIR="${OUT_DIR}/raw_mac"
SUMMARY_DIR="${OUT_DIR}/summary"
mkdir -p "${RAW_DIR}" "${SUMMARY_DIR}"

# 文件输出
RUN_TSV="${SUMMARY_DIR}/runs_mac_${RUN_TAG}.tsv"
GROUP_TSV="${SUMMARY_DIR}/group_summary_mac_${RUN_TAG}.tsv"
...
# Mac连接自己的A节点（作为客户端）
PEER_ADDR="${MAC_IP}:${MAC_BASE_PORT}"
...
run_download() {
  ...
  for ((r=1; r<=ROUNDS; r++)); do
    ...
    for ((i=START_IDX; i<=END_IDX; i++)); do
      ...
      ) > "${PRO_DIR}/tmp_mac_job_${r}_${idx}.status" &
    ...
    for ((i=START_IDX; i<=END_IDX; i++)); do
      ...
      local status_file="${PRO_DIR}/tmp_mac_job_${r}_${j}.status"
